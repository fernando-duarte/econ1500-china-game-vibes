        ...
        if (fs.existsSync(portFile)) {
          ...
          if (port && !isNaN(port)) {
            ...
            try {
              ...
              reject(
                new Error(`Server started but waitOn failed: ${error.message}`),
              );
              return false;
            }
          }
          ...
          serverProcess.stdout.on('data', async (data) => {
            ...
            if (match && match[1]) {
              ...
              try {
                ...
                reject(
                  new Error(
                    `Server started but waitOn failed: ${error.message}`,
                  ),
                );
              }
            }
...
async function waitForElement(page, selector, timeout = 5000) {
  try {
    ...
    await takeDebugScreenshot(
      page,
      `element-not-found-${selector.replace(/[^a-zA-Z0-9]/g, '-')}`,
    );
    return null;
  }
}
  ...
  for (const browser of activeBrowsers) {
    closingPromises.push(
      browser
        .close()
        .catch((err) =>
          console.error('Error closing browser in cleanup:', err),
        ),
    );
    activeBrowsers.delete(browser);
  }
