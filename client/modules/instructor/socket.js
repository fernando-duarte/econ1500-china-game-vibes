...
(function(window) {
  ...
  const InstructorSocket = {
    ...
    registerGameManagementEvents: function() {
      this.socket.on(CONSTANTS.SOCKET.EVENT_GAME_CREATED, this.handleGameCreated.bind(this));
      ...
      this.socket.on(CONSTANTS.SOCKET.EVENT_MANUAL_START_MODE, this.handleManualStartMode.bind(this));
    },
    ...
    registerPlayerEvents: function() {
      this.socket.on(CONSTANTS.SOCKET.EVENT_PLAYER_JOINED, this.handlePlayerJoined.bind(this));
      this.socket.on(CONSTANTS.SOCKET.EVENT_PLAYER_DISCONNECTED, this.handlePlayerDisconnected.bind(this));
    },
    ...
    registerRoundEvents: function() {
      ...
      this.socket.on(CONSTANTS.SOCKET.EVENT_ROUND_SUMMARY, this.handleRoundSummary.bind(this));
      this.socket.on(CONSTANTS.SOCKET.EVENT_TIMER_UPDATE, this.handleTimerUpdate.bind(this));
    },
    ...
    registerNotificationEvents: function() {
      ...
      this.socket.on(CONSTANTS.SOCKET.EVENT_ADMIN_NOTIFICATION, this.handleAdminNotification.bind(this));
    },
    ...
    handleRoundStart: function(data) {
      ...
      if (data.roundNumber > CONSTANTS.FIRST_ROUND_NUMBER && elements.roundResultsSection) {
    ...
    handleInvestmentReceived: function(data) {
      ...
      const elements = InstructorDom.elements;
      if (elements.currentInvestmentsSection && 
          elements.currentInvestmentsSection.classList.contains(CONSTANTS.CSS.HIDDEN)) {
        elements.currentInvestmentsSection.classList.remove(CONSTANTS.CSS.HIDDEN);
      }
      ...
      const formattedInvestment = SocketUtils.formatNumber(data.investment, CONSTANTS.DECIMAL_PRECISION);
    ...
    handleAllPlayersSubmitted: function() {
      ...
      if (elements.roundNumber && 
          elements.roundNumber.textContent === String(CONSTANTS.FIRST_ROUND_NUMBER - 1)) {
    ...
    createResultRow: function(result) {
      ...
      const formattedInvestment = SocketUtils.formatNumber(result.investment, CONSTANTS.DECIMAL_PRECISION);
      const autoSubmitSuffix = result.isAutoSubmit ? CONSTANTS.UI_TEXT.AUTO_SUBMIT_SUFFIX : '';
