...
window.screenSocket = {
  ...
  registerEventHandlers: function() {
    ...
    this.socket.on(CONSTANTS.SOCKET.EVENT_CONNECT, this.handlers.onConnect.bind(this));
    this.socket.on(CONSTANTS.SOCKET.EVENT_DISCONNECT, this.handlers.onDisconnect.bind(this));
    
    // Group: Player events
    this.socket.on(CONSTANTS.SOCKET.EVENT_PLAYER_JOINED, this.handlers.onPlayerJoined.bind(this));
    this.socket.on(CONSTANTS.SOCKET.EVENT_PLAYER_DISCONNECTED, this.handlers.onPlayerDisconnected.bind(this));
    
    // Group: Game state events
    this.socket.on(CONSTANTS.SOCKET.EVENT_GAME_CREATED, this.handlers.onGameCreated.bind(this));
    this.socket.on(CONSTANTS.SOCKET.EVENT_GAME_STARTED, this.handlers.onGameStarted.bind(this));
    this.socket.on(CONSTANTS.SOCKET.EVENT_ROUND_START, this.handlers.onRoundStart.bind(this));
    this.socket.on(CONSTANTS.SOCKET.EVENT_INVESTMENT_RECEIVED, this.handlers.onInvestmentReceived.bind(this));
    this.socket.on(CONSTANTS.SOCKET.EVENT_ALL_SUBMITTED, this.handlers.onAllSubmitted.bind(this));
    this.socket.on(CONSTANTS.SOCKET.EVENT_ROUND_SUMMARY, this.handlers.onRoundSummary.bind(this));
    this.socket.on(CONSTANTS.SOCKET.EVENT_GAME_OVER, this.handlers.onGameOver.bind(this));
    
    // Group: Utility events
    this.socket.on(CONSTANTS.SOCKET.EVENT_STATE_SNAPSHOT, this.handlers.onStateSnapshot.bind(this));
    this.socket.on(CONSTANTS.SOCKET.EVENT_TIMER_UPDATE, this.handlers.onTimerUpdate.bind(this));
    this.socket.on(CONSTANTS.SOCKET.EVENT_ERROR, this.handlers.onError.bind(this));
    this.socket.on(CONSTANTS.SOCKET.EVENT_INSTRUCTOR_DISCONNECTED, this.handlers.onInstructorDisconnected.bind(this));
  }
};
...
window.screenSocket.handlers = {
  ...
  onRoundStart: function(data) {
    ...
    if (elements.gameStatus) {
      elements.gameStatus.textContent = CONSTANTS.UI_TEXT.STATUS_ROUND_IN_PROGRESS;
    }
  ...
  onRoundSummary: function(data) {
    ...
    // Update game status
    if (elements.gameStatus) {
      elements.gameStatus.textContent = CONSTANTS.UI_TEXT.ROUND_COMPLETED_FORMAT.replace('{0}', data.roundNumber);
    }
  ...
  onStateSnapshot: function(data) {
    ...
    if (data.roundNumber >= CONSTANTS.FIRST_ROUND_NUMBER && elements.gameStatus) {
      elements.gameStatus.textContent = CONSTANTS.UI_TEXT.STATUS_ROUND_IN_PROGRESS;
      window.screenGame.updateGameState(CONSTANTS.GAME_STATES.ACTIVE);
    }
